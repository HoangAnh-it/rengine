...
class OllamaManager(APIView):
    def get(self, request):
        ...
        try:
            ...
            _response = requests.post(pull_model_api, json={"name": model_name, "stream": False}).json()
    ...
    def delete(self, request):
        ...
        response = {"status": False}
        try:
            _response = requests.delete(delete_model_api, json={"name": model_name}).json()
            if _response.get("error"):
                response["status"] = False
                response["error"] = _response.get("error")
            else:
    ...
    def put(self, request):
        ...
        if any(model["name"] == model_name for model in DEFAULT_GPT_MODELS):
            use_ollama = False
        try:
            OllamaSettings.objects.update_or_create(defaults={"selected_model": model_name, "use_ollama": use_ollama}, id=1)
            response["status"] = True
        except Exception as e:
            response["error"] = str(e)
        return Response(response)
...
class GPTAttackSuggestion(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response({"status": False, "error": "Subdomain not found with id " + subdomain_id})
        if subdomain.attack_surface:
            return Response({"status": True, "subdomain_name": subdomain.name, "description": subdomain.attack_surface})
...
class ListTargetsDatatableViewSet(viewsets.ModelViewSet):
    ...
    def filter_queryset(self, qs):
        ...
        if search_value or _order_col or _order_direction:
            ...
            qs = self.queryset.filter(Q(name__icontains=search_value) | Q(description__icontains=search_value) | Q(domains__name__icontains=search_value))
...
class UniversalSearch(APIView):
    def get(self, request):
        ...
        subdomain = Subdomain.objects.filter(Q(name__icontains=query) | Q(cname__icontains=query) | Q(page_title__icontains=query) | Q(http_url__icontains=query)).distinct("name")
        subdomain_data = SubdomainSerializer(subdomain, many=True).data
        response["results"]["subdomains"] = subdomain_data

        endpoint = EndPoint.objects.filter(Q(http_url__icontains=query) | Q(page_title__icontains=query)).distinct("http_url")
        endpoint_data = EndpointSerializer(endpoint, many=True).data
        response["results"]["endpoints"] = endpoint_data

        vulnerability = Vulnerability.objects.filter(Q(http_url__icontains=query) | Q(name__icontains=query) | Q(description__icontains=query)).distinct()
    ...
    def post(self, request):
        ...
        try:
            ...
            if scan_history_id:
                ...
                if is_ignore_info:
                    most_common_vulnerabilities = vuln_query.exclude(severity=0).annotate(count=Count("name")).order_by("-count")[:limit]
                else:
        ...
        if scan_history_id:
            ...
            if is_ignore_info:
                most_vulnerable_subdomains = (
                    subdomain_query.annotate(vuln_count=Count("vulnerability__name", filter=~Q(vulnerability__severity=0))).order_by("-vuln_count").exclude(vuln_count=0)[:limit]
                )
            else:
                most_vulnerable_subdomains = subdomain_query.annotate(vuln_count=Count("vulnerability__name")).order_by("-vuln_count").exclude(vuln_count=0)[:limit]
            ...
            if is_ignore_info:
                most_vulnerable_targets = (
                    domains.annotate(vuln_count=Count("subdomain__vulnerability__name", filter=~Q(subdomain__vulnerability__severity=0))).order_by("-vuln_count").exclude(vuln_count=0)[:limit]
                )
            else:
                most_vulnerable_targets = domains.annotate(vuln_count=Count("subdomain__vulnerability__name")).order_by("-vuln_count").exclude(vuln_count=0)[:limit]
    ...
    def post(self, request):
        ...
        if organization_name:
            ...
            if organization_query.exists():
                ...
                organization_obj = Organization.objects.create(name=organization_name, project=project, insert_date=timezone.now())
            organization_obj.domains.add(domain)

        return Response({"status": True, "message": "Domain successfully added as target !", "domain_name": domain_name, "domain_id": domain.id})
    ...
    def post(self, request):
        ...
        for subdomain_id in data["subdomain_ids"]:
            ...
            for stype in scan_types:
                ctx = {"scan_history_id": None, "subdomain_id": subdomain_id, "scan_type": stype, "engine_id": engine_id}
...
class RengineUpdateCheck(APIView):
    def get(self, request):
        ...
        current_version = ((os.environ["RENGINE_CURRENT_VERSION"])[1:] if os.environ["RENGINE_CURRENT_VERSION"][0] == "v" else os.environ["RENGINE_CURRENT_VERSION"]).replace("'", "")

        # for consistency remove v from both if exists
        latest_version = re.search(r"v(\d+\.)?(\d+\.)?(\*|\d+)", ((response[0]["name"])[1:] if response[0]["name"][0] == "v" else response[0]["name"]))

        latest_version = latest_version.group(0) if latest_version else None

        if not latest_version:
            latest_version = re.search(r"(\d+\.)?(\d+\.)?(\*|\d+)", ((response[0]["name"])[1:] if response[0]["name"][0] == "v" else response[0]["name"]))
            if latest_version:
...
class ScanStatus(APIView):
    def get(self, request):
        ...
        # main tasks
        recently_completed_scans = ScanHistory.objects.filter(domain__project__slug=slug).order_by("-start_scan_date").filter(Q(scan_status=0) | Q(scan_status=2) | Q(scan_status=3))[:10]
        current_scans = ScanHistory.objects.filter(domain__project__slug=slug).order_by("-start_scan_date").filter(scan_status=1)
        pending_scans = ScanHistory.objects.filter(domain__project__slug=slug).filter(scan_status=-1)

        # subtasks
        recently_completed_tasks = SubScan.objects.filter(scan_history__domain__project__slug=slug).order_by("-start_scan_date").filter(Q(status=0) | Q(status=2) | Q(status=3))[:15]
        current_tasks = SubScan.objects.filter(scan_history__domain__project__slug=slug).order_by("-start_scan_date").filter(status=1)
...
class ListTechnology(APIView):
    def get(self, request, format=None):
        ...
        if target_id:
            tech = Technology.objects.filter(technologies__in=Subdomain.objects.filter(target_domain__id=target_id)).annotate(count=Count("name")).order_by("-count")
...
class ListDorkTypes(APIView):
    def get(self, request, format=None):
        ...
        scan_id = req.query_params.get("scan_id")
        if scan_id:
            dork = Dork.objects.filter(dorks__in=ScanHistory.objects.filter(id=scan_id)).values("type").annotate(count=Count("type")).order_by("-count")
            serializer = DorkCountSerializer(dork, many=True)
            return Response({"dorks": serializer.data})
        else:
            dork = Dork.objects.filter(dorks__in=ScanHistory.objects.all()).values("type").annotate(count=Count("type")).order_by("-count")
            serializer = DorkCountSerializer(dork, many=True)
            return Response({"dorks": serializer.data})
...
class ListPorts(APIView):
    def get(self, request, format=None):
        ...
        if target_id:
            port = Port.objects.filter(ports__in=IpAddress.objects.filter(ip_addresses__in=Subdomain.objects.filter(target_domain__id=target_id))).distinct()
        elif scan_id:
...
class ListSubdomains(APIView):
    def get(self, request, format=None):
        ...
        if port:
            subdomain_query = subdomain_query.filter(ip_addresses__in=IpAddress.objects.filter(ports__in=Port.objects.filter(number=port)))
...
class ListIPs(APIView):
    def get(self, request, format=None):
        ...
        if target_id:
            ips = IpAddress.objects.filter(ip_addresses__in=Subdomain.objects.filter(target_domain__id=target_id)).distinct()
        elif scan_id:
...
class SubdomainChangesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        scan_history_query = ScanHistory.objects.filter(domain=domain_id).filter(tasks__overlap=["subdomain_discovery"]).filter(id__lte=scan_id).exclude(Q(scan_status=-1) | Q(scan_status=1))
...
class EndPointChangesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        scan_history = ScanHistory.objects.filter(domain=domain_id).filter(tasks__overlap=["fetch_url"]).filter(id__lte=scan_id).filter(scan_status=2)
...
class InterestingSubdomainViewSet(viewsets.ModelViewSet):
    ...
    def filter_queryset(self, qs):
        ...
        if search_value:
            qs = self.queryset.filter(Q(name__icontains=search_value) | Q(page_title__icontains=search_value) | Q(http_status__icontains=search_value))
        return qs.order_by(order_col)
    ...
    def special_lookup(self, search_value):
        ...
        if "=" in search_value:
            ...
            if "severity" in lookup_title:
                ...
                qs = self.queryset.filter(Q(description__icontains=lookup_content) | Q(template__icontains=lookup_content) | Q(extracted_results__icontains=lookup_content))
        elif "!" in search_value:
            search_param = search_value.split("!")
            lookup_title = search_param[0].lower().strip()
            lookup_content = search_param[1].lower().strip()
            if "severity" in lookup_title:
                ...
                qs = self.queryset.exclude(Q(description__icontains=lookup_content) | Q(template__icontains=lookup_content) | Q(extracted_results__icontains=lookup_content))
