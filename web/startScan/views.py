...
def detail_scan(request, id, slug):
    ...
    last_scans = ScanHistory.objects.filter(domain__id=domain_id).filter(tasks__overlap=["subdomain_discovery"]).filter(id__lte=id).filter(scan_status=2)
    ...
    alive_count = subdomains.values("name").distinct().filter(http_status__exact=200).count()
    ...
    endpoint_alive_count = endpoints.filter(http_status__exact=200).values("http_url").distinct().count()  # TODO: use is_alive() func as it's more precise

    # Vulnerabilities
    common_vulns = vulns.exclude(severity=0).values("name", "severity").annotate(count=Count("name")).order_by("-count")[:10]
...
def start_scan_ui(request, slug, domain_id):
    ...
    context = {"scan_history_active": "active", "domain": domain, "engines": engine, "custom_engine_count": custom_engine_count}
...
def start_multiple_scan(request, slug):
    ...
    context = {"scan_history_active": "active", "engines": engines, "domain_list": list_of_domain_name, "domain_ids": domain_ids, "custom_engine_count": custom_engine_count}
...
def schedule_scan(request, host_id, slug):
    ...
    if request.method == "POST":
        ...
        if scheduled_mode == "periodic":
            ...
            PeriodicTask.objects.create(interval=schedule, name=task_name, task="initiate_scan", kwargs=json.dumps(kwargs))
            ...
            PeriodicTask.objects.create(clocked=clock, one_off=True, name=task_name, task="initiate_scan", kwargs=json.dumps(kwargs))
    ...
    engines = EngineType.objects
    custom_engine_count = engines.filter(default_engine=False).count()
    context = {"scan_history_active": "active", "domain": domain, "engines": engines, "custom_engine_count": custom_engine_count}
...
def start_organization_scan(request, id, slug):
    ...
    if request.method == "POST":
        ...
        messages.add_message(request, messages.INFO, f"Scan Started for {ndomains} domains in organization {organization.name}")
        return HttpResponseRedirect(reverse("scan_history", kwargs={"slug": slug}))
...
def schedule_organization_scan(request, slug, id):
    ...
    if request.method == "POST":
        ...
        for domain in organization.get_domains():
            ...
            if scheduled_mode == "periodic":
                ...
                schedule, _ = IntervalSchedule.objects.get_or_create(every=frequency_value, period=period)
                _kwargs = json.dumps(
                    {"domain_id": domain.id, "engine_id": engine.id, "scan_history_id": 0, "scan_type": SCHEDULED_SCAN, "imported_subdomains": None, "initiated_by_id": request.user.id}
                )
            ...
            # Clocked task
                ...
                schedule_time = request.POST["scheduled_time"]
                clock, _ = ClockedSchedule.objects.get_or_create(clocked_time=schedule_time)
                _kwargs = json.dumps({"domain_id": domain.id, "engine_id": engine.id, "scan_history_id": 0, "scan_type": LIVE_SCAN, "imported_subdomains": None, "initiated_by_id": request.user.id})
                PeriodicTask.objects.create(clocked=clock, one_off=True, name=task_name, task="initiate_scan", kwargs=_kwargs)
    ...
    custom_engine_count = EngineType.objects.filter(default_engine=False).count()
    context = {"scan_history_active": "active", "organization": organization, "domain_list": organization.get_domains(), "engines": engine, "custom_engine_count": custom_engine_count}
...
def create_report(request, id):
    ...
    unique_vulns = (
        (Vulnerability.objects.filter(scan_history=scan).values("name", "severity").annotate(count=Count("name")).order_by("-severity", "-count"))
        if not is_ignore_info_vuln
        else (Vulnerability.objects.filter(scan_history=scan).exclude(severity=0).values("name", "severity").annotate(count=Count("name")).order_by("-severity", "-count"))
    )

    subdomains = Subdomain.objects.filter(scan_history=scan).order_by("-content_length")
    subdomain_alive_count = Subdomain.objects.filter(scan_history__id=id).values("name").distinct().filter(http_status__exact=200).count()
