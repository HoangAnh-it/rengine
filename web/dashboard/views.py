...
def index(request, slug):
    ...
    total_vul_count = info_count + low_count + medium_count + high_count + critical_count + unknown_count
    total_vul_ignore_info_count = low_count + medium_count + high_count + critical_count
    last_week = timezone.now() - timedelta(days=7)

    count_targets_by_date = domains.filter(insert_date__gte=last_week).annotate(date=TruncDay("insert_date")).values("date").annotate(created_count=Count("id")).order_by("-date")
    count_subdomains_by_date = subdomains.filter(discovered_date__gte=last_week).annotate(date=TruncDay("discovered_date")).values("date").annotate(count=Count("id")).order_by("-date")
    count_vulns_by_date = vulnerabilities.filter(discovered_date__gte=last_week).annotate(date=TruncDay("discovered_date")).values("date").annotate(count=Count("id")).order_by("-date")
    count_scans_by_date = scan_histories.filter(start_scan_date__gte=last_week).annotate(date=TruncDay("start_scan_date")).values("date").annotate(count=Count("id")).order_by("-date")
    count_endpoints_by_date = endpoints.filter(discovered_date__gte=last_week).annotate(date=TruncDay("discovered_date")).values("date").annotate(count=Count("id")).order_by("-date")
...
def on_user_logged_out(sender, request, **kwargs):
    messages.add_message(request, messages.INFO, "You have been successfully logged out. Thank you " + "for using reNgine.")


@receiver(user_logged_in)
def on_user_logged_in(sender, request, **kwargs):
    messages.add_message(request, messages.INFO, "Hi @" + request.user.username + " welcome back!")
